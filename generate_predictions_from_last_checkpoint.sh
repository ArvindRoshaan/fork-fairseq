#!/bin/bash
#generate predictions for data splits train, valid and test using the last checkpoint
...
if [ $is_test -ne 1 ]; then
    ...
    #Generate output
    fairseq-generate data-bin/$model_folder_name --path checkpoints/$model_folder_name/$model_desc/checkpoint_last.pt --batch-size 128 --beam 5 --remove-bpe --skip-invalid-size-inputs-valid-test > output_custom_fldr/$model_folder_name/$model_desc/output_test_lastCP_$lang.txt
fi
...
if [ $is_train -ne 1 ]; then
    ...
    #Generate output
    fairseq-generate data-bin/$model_folder_name --path checkpoints/$model_folder_name/$model_desc/checkpoint_last.pt --batch-size 128 --beam 5 --remove-bpe --skip-invalid-size-inputs-valid-test > output_custom_fldr/$model_folder_name/$model_desc/output_train_lastCP_$lang.txt
...
if [ $is_valid -ne 1 ]; then
    ...
    #Generate output
    fairseq-generate data-bin/$model_folder_name --path checkpoints/$model_folder_name/$model_desc/checkpoint_last.pt --batch-size 128 --beam 5 --remove-bpe --skip-invalid-size-inputs-valid-test > output_custom_fldr/$model_folder_name/$model_desc/output_valid_lastCP_$lang.txt
